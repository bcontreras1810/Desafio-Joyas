...
const orderValues = (order) => {
  return order == "asc"
    ? joyas.results.slice().sort((a, b) => a.value - b.value)
    : order == "desc"
      ? joyas.results.slice().sort((a, b) => b.value - a.value)
      : false;
};
...
const filtroCategory = (category) => {
  const joyasFiltradas = joyas.results.filter(joya => joya.category === category);
